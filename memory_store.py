...
memory_store.py
----------------
Two layers of memory for the support agent:

1. ShortTermMemory  -> per-session conversation history (rolling window),
                        kept in RAM, reset each session.
2. LongTermMemory    -> persisted to disk as JSON, keyed by user_id.
                        Stores facts the agent learns about a user across
                        sessions (e.g. preferred category, last order asked
                        about, open return case, sentiment flags) plus a
                        durable interaction log.

This is intentionally dependency-free (no vector DB) so the project runs
anywhere with just the standard library. Swap `LongTermMemory` internals
for a real DB / vector store in production without touching the agent code,
since the agent only talks to the public methods below.
"""
